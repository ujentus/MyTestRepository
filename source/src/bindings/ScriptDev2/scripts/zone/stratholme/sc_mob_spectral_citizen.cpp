...
#define SPELL_DEBILITATINGTOUCH    16333
#define SPELL_HAUNTINGPHANTOMS    16336
#define SPELL_DAZED    1604
#define SPELL_DEMORALIZINGSHOUT    23511

struct MANGOS_DLL_DECL mob_spectral_citizenAI : public ScriptedAI
{
    mob_spectral_citizenAI(Creature *c) : ScriptedAI(c) {EnterEvadeMode();}

    uint32 DebilitatingTouch_Timer;
    ...
    void EnterEvadeMode()
    {
        DebilitatingTouch_Timer = 6000;
        HauntingPhantoms_Timer = 9000;
        Dazed_Timer = 12000;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if( m_creature->getVictim() && m_creature->isAlive())
        {
            ...
            //DebilitatingTouch
            if (DebilitatingTouch_Timer < diff)
            {
                //Cast
                if (rand()%100 < 55) //55% chance to cast
                {
                    DoCast(m_creature->getVictim(),SPELL_DEBILITATINGTOUCH);
                }
                //15 seconds until we should cast this again
                DebilitatingTouch_Timer = 15000;
            }else DebilitatingTouch_Timer -= diff;
...
CreatureAI* GetAI_mob_spectral_citizen(Creature *_Creature)
{
    return new mob_spectral_citizenAI (_Creature);
}

bool ReciveEmote_mob_spectral_citizen(Player *player, Creature *_Creature, uint32 emote)
{
    _Creature->HandleEmoteCommand(emote);

    if (emote == EMOTE_STATE_DANCE)
        ((mob_spectral_citizenAI*)&_Creature->AI())->EnterEvadeMode();

    return true;
}

void AddSC_mob_spectral_citizen()
{
    Script *newscript;
    newscript = new Script;
    newscript->Name="mob_spectral_citizen";
    newscript->GetAI = GetAI_mob_spectral_citizen;
    newscript->pReceiveEmote = &ReciveEmote_mob_spectral_citizen;
