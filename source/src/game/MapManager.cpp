...
{
    sDatabase.PExecute("DELETE FROM `%s`", dest_tbl);

    //sDatabase.PExecute("DROP INDEX `idx_search` ON `%s`", dest_tbl);
	sDatabase.PExecute("DROP INDEX IF EXISTS `idx_search_%s`", dest_tbl) ;
    ...
    sDatabase.PExecute(
        ...
        "SELECT `guid`,`map`,round(((`position_x`-'%f')/'%f') + '%u'), round(((`position_y`-'%f')/'%f') + '%u'),"
        "round(((`position_x`-'%f')/'%f') + '%u'), round(((`position_y`-'%f')/'%f') + '%u')  FROM `%s`",
...
//	sDatabase.PExecute("CREATE INDEX `idx_search` ON `%s` (`grid`,`cell`,`map`)", dest_tbl);
	sDatabase.PExecute("CREATE INDEX `idx_search_%s` ON `%s` (`grid`,`cell`,`map`)", dest_tbl, dest_tbl) ;
...
MapManager::~MapManager()
{
    ...
    sDatabase.PExecute("DELETE FROM `creature_grid`");
    sDatabase.PExecute("DELETE FROM `gameobject_grid`");
    sDatabase.PExecute("DELETE FROM `corpse_grid`");
