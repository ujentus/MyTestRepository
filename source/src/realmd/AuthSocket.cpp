...
#include "Auth/crypt/md5.h"
#include "Auth/Sha1.h"

extern RealmList m_realmList;
extern DatabaseSqlite dbRealmServer;
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
		...
		dbRealmServer.PExecute("UPDATE `account` SET `sessionkey` = '%s', `last_ip` = '%s', `last_login` = datetime('now') WHERE `username` = '%s'",K.AsHexStr(), GetRemoteAddress().c_str(), _safelogin.c_str() );
