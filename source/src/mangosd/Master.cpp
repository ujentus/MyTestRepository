...
void Master::Run()
{
    sLog.outString( "MaNGOS daemon %s for WOW 1.12.x", _FULLVERSION );
...
bool Master::_StartDB()
{
    ...
    if(!sDatabase.Initialize("./database/mangos.db3"))
    {
        sLog.outError("Cannot connect to world database ./database/mangos.db3") ;
        return false;
    }

    ///- Initialise the login database
    if(!loginDatabase.Initialize("./database/realmd.db3"))
    {
        sLog.outError("Cannot connect to login database ./database/realmd.db3") ;
        return false;
    }
